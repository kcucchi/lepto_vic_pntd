...
# check statistics of count data

mean(log(df_cnt_week_subset$count+1),na.rm = T)

var(log(df_cnt_week_subset$count+1),na.rm = T)
...
names_vic = c('vic_roff');suffix_pdf="roff"
# names_vic = c('vic_sm1');suffix_pdf="sm1"
...
# this is for temperature only
# names_vic = ""; suffix_pdf = ""

# if true, temperature at a 2 lag will be included in the regression
# if false, no temperature is included in the regression
T2weeks_boolean <- F
...
if(T2weeks_boolean){
  if(nchar(names_vic[1]) > 0){
    formula_vic_all <- 
      paste0("count ~ ",
             paste(paste0('cb_', names_vic, '_subset'), collapse = ' + '),
             # 2 weeks is v1.l3
             " + cb_vic_Tmin_subset[,'v1.l3'] + CNTY_CODE + ob.trend + offset(log(pop_total))")   
  }else{
    formula_vic_all <- 
      paste0("count ~ ",
             # 2 weeks is v1.l3
...
# annoying - if there is one variable, the output is a vector of length x
# and not a 1-by-x array...........  need to fix that
# in that case it's necessarily temperature
if(is.null(dim(conf_int))){
  conf_int <- array(conf_int,
                    dim=c(1,length(conf_int)),
                    dimnames = list('cb_vic_Tmin_subset[, "v1.l3"]',
                                    names(conf_int)))
}
...
df_vif <- data.frame(car::vif(glm_multiple))
names(df_vif) <- c("GVIF", "Df", "GVIF^(1/(2*Df))")
# rename variables in line with original nomenclature
rownames(df_vif) <- str_remove(string = rownames(df_vif), pattern = 'cb_')
...
g_beta <-
  ggplot(data = filter(df_glm_multiple,
                       var %in% c('vic_Prcp', 'vic_sm1', 'vic_roff')),
...
df_quartiles <-
  data.frame(
    var=c("vic_Prcp", "vic_sm1", "vic_roff", "vic_Tmin"),
    var_quart1=unlist(lapply(
      X = df_cnt_week_subset[, c("vic_Prcp", "vic_sm1",
                                 "vic_roff", "vic_Tmin")],
...
write.table(x = df_glm_multiple[,c('var','lag',
                                   "var_quart1","var_quart3","var_interquart", 
                                   'text_beta','text_RR','text_p_value','p_value')],
            file = paste0('table_coefficients_', suffix_pdf,
                          '_bool_', T2weeks_boolean, '_vif_T2weeks.csv'),
            sep = ',',
            row.names = F)

# save file with vif values

df_vif

df_vif$`GVIF^(1/Df)` <- df_vif$`GVIF^(1/(2*Df))`^2

write.table(x = round(df_vif, 2),
            file = paste0('vif_values_', suffix_pdf,
                          '_bool_', T2weeks_boolean, '_vif_T2weeks.csv'),
            sep = ',')
